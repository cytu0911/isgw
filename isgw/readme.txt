编译安装指导：
1 编译(需要先编译和isgw平级的 comm 目录)：进入 svr 目录,  make conf; make
2 安装(可使用install.sh脚本进行自动安装)：建议目录结构为 bin cfg log 
3 启动 ： 使用 start.sh 脚本进行即可 
4 测试 ： telnet 或者 用 client 目录的 测试工具 
...
3 异步连接管理器支持自定义消息id(用于唯一确定一条异步消息)解析函数，将来可以支持idip等其他的后端平台或者服务
...
1 优化对象池回收机制，尽早进行对象回收，不等到网络数据发送完成，
  尽早减少内存占用，能减少 "ACE_Object_Que dequeue failed" 出现的次数 
...
打包命令 :
tar -czvf isgw_v3.3d202.tgz comm isgw 
